const {whatsnext, whatsnextcategory} = require('../models');
...
const create = async (req, res) => {
    ...
    upload(req, res, function (err) {
        ...
        whatsnext.create(body).then(result => {
            try {
...
const getAll = async (req, res) => {
    ...
    [err, attachment] = await to(whatsnext.findAll({
        include: [{model: whatsnextcategory}]
    }));
...
const remove    =   async function (req, res) {
    let attach, err ;
    let id = req.params.id;
    [err, attach] = await to(whatsnext.destroy({
        where: {id:id}
    }));
